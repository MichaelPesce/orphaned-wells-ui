...
When ready, click the upload or batch-processing button for the selected method. Local directory files transfer directly from your browser to GCS before processing starts. Keep the dialog open until transfer finishes. Processing then continues in a worker, and **Recent processing jobs** shows progress and failures. Reopen the upload dialog to check progress after a refresh.
