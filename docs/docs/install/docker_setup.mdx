...
With the default Docker settings, authentication is disabled, files are stored in the backend Docker volume, and schemas come from the installed `ogrre_data_cleaning` package in read-only mode. Set `USE_DB_PROCESSORS=true` explicitly to use editable, database-backed schemas. These defaults are suitable for local development and CI, but not for production.

If you enable Google Cloud integrations, add the relevant project, bucket, service-account, and OAuth values in `deployment/.env`. See [Create Google service-account keys](gcp_setup#create-google-service-account-keys) for the storage and Document AI runtime keys used by local Docker.

For Google-backed local Docker runs, set the runtime key paths in `deployment/.env`:
...
Put those key files next to `deployment/.env` or use absolute paths. The Docker start scripts mount each file into the backend container separately.

For directory uploads using the staging bucket (`uploaded_documents_v0`),
Terraform includes `http://localhost:3000` and `http://localhost:3001` in the
bucket CORS defaults. Normal Docker development uses port 3000; the isolated
E2E stack uses port 3001. The local backend's `ALLOWED_ORIGINS` must include the
origin you open in the browser. See [upload bucket configuration](../deploy-gcp/backend-gke.mdx#directory-upload-storage-configuration)
for other hosts or ports. With `STORAGE_BACKEND=local`, bucket CORS is not used.
...
The Cypress E2E suite uses the same sample dump for local and CI runs. Local E2E runs should use the isolated `npm run docker:e2e` stack, which restores into `DB_NAME=isgs_e2e` by default. See [Testing](/docs/development/testing) and [E2E CI](/docs/development/e2e-ci) for the test commands and reset behavior.
...
If you change `FRONTEND_HOST_PORT`, also set `ALLOWED_ORIGINS` to the frontend's
browser origin, for example `http://localhost:3001`. The browser uses the
published host port even though the frontend listens on port 3000 inside Docker.
For Google-backed development, keep the bucket CORS origin list aligned as
documented above. Opening `http://127.0.0.1:3001` requires that exact origin;
it does not match `http://localhost:3001`.
