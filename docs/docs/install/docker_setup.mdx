...
If you enable Google Cloud integrations, add the relevant project, bucket, service account, and OAuth values in `deployment/.env`. See [Create a Google Cloud Storage service-account key](gcp_setup#create-a-google-cloud-storage-service-account-key) for the Cloud Storage key setup.
