...
Open **Upload history** from the dialog footer or the record group's actions
menu. The page separates **Active uploads** from **Finished uploads**, each with
server pagination (25 jobs per page). Finished-job filters cover status, source,
uploader, and submission date; they never hide active uploads. Jobs are shown
newest first. An older active job remains accessible even after many newer jobs.
...
polling; use **Refresh uploads** in the page actions or **Refresh** in details,
