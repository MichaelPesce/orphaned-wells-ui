...
Collaborator APIs retain 2 replicas with 1850m CPU and 12 GiB memory per pod
while the smaller staging size is validated. Staging targets 1 API replica
with 1 CPU and 4 GiB memory, using two Uvicorn workers. These values set both
requests and limits. Processing-worker resources are configured separately.
...
For an otherwise up-to-date workspace, the staging reduction should change only
the `kubernetes_deploy_targets.staging` output: `memory_request` and
`memory_limit` go from `6Gi` to `4Gi`. It requires no infrastructure resources
to be added, changed, or destroyed. Review any other plan changes separately.
An older `K8S_DEPLOY_TARGETS` secret containing `6Gi` continues to select that
size until refreshed; explicit target values take precedence over workflow
defaults. The updated workflow honors a `4Gi` limit without increasing it.

Follow the [staging validation and rollback steps](./backend-gke.mdx#capacity-failure-recovery-and-resource-sizing)
before reducing production. After validation, apply API overrides to one
production environment at a time, keeping its two replicas and existing worker
settings. The first candidate is 1 CPU / 4 GiB per API pod; 1 CPU / 6 GiB is an
intermediate option if measurements require more headroom.
...
Single-file/ZIP processing, record-image uploads, imports, rotation, and exports
still use API resources. Include these workflows in staging load checks before
choosing a smaller production API allocation.
