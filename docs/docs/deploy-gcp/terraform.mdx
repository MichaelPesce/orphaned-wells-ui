...
Collaborator APIs target 2 replicas with 1 CPU and 4 GiB memory per pod.
...
`kubernetes_deploy_targets` output. Production API `memory_request` and
`memory_limit` go from `6Gi` to `4Gi`; CPU stays at `1`, production replicas
stay at `2`, and worker settings stay unchanged. It requires no infrastructure
resources to be added, changed, or destroyed. Review any other plan changes separately. An older
...
and deploy production environments one at a time at 1 CPU / 4 GiB, keeping two
API replicas and existing worker settings. Validate production behavior before
