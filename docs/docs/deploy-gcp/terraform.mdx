...
Collaborator APIs target 2 replicas with 1 CPU and 6 GiB memory per pod.
Staging targets 1 API replica
...
`kubernetes_deploy_targets` output. Production API `cpu_request` and `cpu_limit`
go from `1850m` to `1`, and `memory_request` and `memory_limit` go from `12Gi` to
`6Gi`. Staging memory also changes from `6Gi` to `4Gi` if that output update has
not already been applied. It requires no infrastructure resources to be added,
changed, or destroyed. Review any other plan changes separately. An older
`K8S_DEPLOY_TARGETS` secret continues to select the old sizes until refreshed;
explicit target values take precedence over workflow defaults. Applying and
exporting these outputs does not resize live pods; a backend deployment
...
and deploy production environments one at a time at 1 CPU / 6 GiB, keeping two
API replicas and existing worker settings. Validate 4 GiB in staging before
considering further production reductions. Use `gke_backend_overrides` for
