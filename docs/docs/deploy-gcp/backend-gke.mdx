...
For other hosts or ports, override the full origin list per bucket, retaining
the default origins you still use. For example, to also use port 3002:
...
origins. See [Docker development](../install/docker_setup.mdx#troubleshooting).
...
their own [CORS handling](https://docs.cloud.google.com/storage/docs/cross-origin#cloud-storage-cors-support).
The browser
...
Staging now targets 1 CPU / 4 GiB for the API, with one replica and two Uvicorn
workers. Its processing workers retain 1 CPU / 6 GiB. Production targets two
API replicas at 1 CPU / 6 GiB each, while its processing workers retain
1850m CPU / 12 GiB.
Include single-file/ZIP uploads, record-image uploads, imports, rotation, and
exports in load checks because those workflows still execute in the API.
Use peak measurements and response times under load to validate production sizing;
an idle usage snapshot is insufficient.

Apply Terraform, refresh `K8S_DEPLOY_TARGETS`, and deploy the affected environments
with the updated workflow. The sizing change updates production API output
requests and limits from 1850m CPU / 12 GiB to 1 CPU / 6 GiB, plus staging memory
from 6 GiB to 4 GiB if that output change has not already been applied. Applying
and exporting the outputs does not resize running pods. Verify admitted pod
resources after deployment because Autopilot can adjust requests:
...
preserving two replicas and all processing-worker settings. Continue validating
4 GiB in staging before considering a further production reduction. Change API
requests and limits together and compare peak usage, throttling, and latency
after each deployment.

To restore staging to its previous allocation, merge this entry into the existing
...
Remove the override when resuming validation at `4Gi`. To restore a production
API to its previous size, set `cpu_request` and `cpu_limit` to `1850m` and
`memory_request` and `memory_limit` to `12Gi` under that environment in
`gke_backend_overrides`, then apply, refresh the secret, and redeploy it. The backend
