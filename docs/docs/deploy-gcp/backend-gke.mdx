...
description: Deploy and operate OGRRE backend environments on GKE.
sidebar_position: 3
---

# Backend on GKE

The current backend deployment path runs each environment as a Kubernetes workload in the shared GKE cluster.

Terraform owns the cloud infrastructure. GitHub Actions renders `orphaned-wells-ui-server/deployment/kubernetes/backend.yaml` with environment-specific values and applies it to GKE.
...
- Optional test DNS records at `<env>-k8s-server.uow-carbon.org`.
- Primary backend DNS records when the environment is listed in `primary_dns_to_gke_backends`.
- `kubernetes_deploy_targets`, the JSON map consumed by GitHub Actions.

Kubernetes creates one namespace per environment, using the `uow-<env>` naming pattern. Each namespace contains:
...
- `SERVICE_KEY_JSON`
- `K8S_DEPLOY_TARGETS`
- One runtime env secret per backend environment, such as `STAGING_ENV`, `CA_ENV`, `ISGS_ENV`, `NEWTS_ENV`, or `OSAGE_ENV`
...
Keep `COLLABORATOR` in the environment secret when the backend needs collaborator-specific processor metadata.
...
Environment-specific workflows deploy an existing image tag, defaulting to `latest`:
