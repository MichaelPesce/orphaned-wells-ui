...
workers. Its processing workers retain 1 CPU / 6 GiB. Production retains two
API replicas at 1850m CPU / 12 GiB each, with the same per-worker allocation.
...
Use peak measurements and response times under load to choose production sizing;
an idle usage snapshot is insufficient.

Apply Terraform, refresh `K8S_DEPLOY_TARGETS`, and deploy staging with the updated
workflow. The sizing change updates only the staging API memory request and
limit in the Terraform output, from `6Gi` to `4Gi`. Verify admitted pod resources
because Autopilot can adjust requests:
...
After validation, deploy one production environment at a time with the measured
API size, preserving two replicas and all processing-worker settings. The first
candidate is 1 CPU / 4 GiB; 1 CPU / 6 GiB is an intermediate option if more
memory headroom is needed. Change API requests and limits together.
...
Remove the override when resuming validation at `4Gi`. The backend
...
- The default collaborator API resource target is 2 replicas at 1850m CPU and 12 GiB memory per pod. Staging targets 1 API replica with 1 CPU and 4 GiB memory; its processing workers retain 1 CPU and 6 GiB.
