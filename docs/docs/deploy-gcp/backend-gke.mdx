...
Staging now targets a 500m CPU / 1 GiB request and a 1 CPU / 2 GiB limit for
the API, with one replica and two Uvicorn workers. Its processing workers
retain 1 CPU / 6 GiB. Production targets two API replicas at 1 CPU / 4 GiB
each, while its processing workers retain 1850m CPU / 12 GiB.
...
with the updated workflow. The sizing change updates staging API output from a
1 CPU / 4 GiB request and limit to a 500m CPU / 1 GiB request and 1 CPU / 2 GiB
limit. Applying and exporting the outputs does not resize running pods. Verify admitted pod
...
1 CPU / 4 GiB per API pod, preserving two replicas and all processing-worker
settings. Change API requests and limits together and compare peak usage,
throttling, and latency after each deployment.
