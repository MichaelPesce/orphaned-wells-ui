...
user submits a GCS batch, the API stores durable job state in MongoDB and
...
The first rollout applies only to GCS-backed batch processing. Single-file and
ZIP uploads still use API Pods, so retain the current API memory requests until
those workflows are moved to workers.
