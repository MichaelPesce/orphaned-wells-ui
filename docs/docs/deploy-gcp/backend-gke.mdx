...
API replicas at 1 CPU / 4 GiB each, while its processing workers retain
1850m CPU / 12 GiB.
...
requests and limits from 1 CPU / 6 GiB to 1 CPU / 4 GiB. Applying and exporting
the outputs does not resize running pods. Verify admitted pod
...
preserving two replicas and all processing-worker settings. Change API requests
