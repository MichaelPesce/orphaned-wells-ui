...
description: Deploy OGRRE frontend environments to Google App Engine.
sidebar_position: 4
---
...
Each frontend environment has its own App Engine service, branch-triggered GitHub Actions workflow, backend URL secret, dispatch route, DNS records, custom domain, and OAuth configuration.

Use `<state>` as the short environment name, such as `ca`, `isgs`, `newts`, `osage`, or `rrc`.
...
Copy an existing `app-*.yaml` file and update the `service` value. Existing services use the `<state>-uow` naming pattern:
...
```text
https://<state>-server.uow-carbon.org
```
...
- `REACT_APP_STATE`
- `REACT_APP_COLLABORATOR`, when configured
...
Then add `<state>.uow-carbon.org` in App Engine custom domains.
...
- Visit `https://<state>.uow-carbon.org`.
