...
:::warning
Never commit secret values, service account keys, OAuth credentials, or environment files to either repository.
:::
...
| `K8S_DEPLOY_TARGETS` | GKE deployment | JSON from `terraform output -json kubernetes_deploy_targets \| jq -c .`. Update it after Terraform changes deployment targets. |
| `<ENV>_ENV` | GKE deployment | Runtime `.env` content for each backend environment, such as `STAGING_ENV`, `CA_ENV`, `ISGS_ENV`, `NEWTS_ENV`, and `OSAGE_ENV`. |
| `REFRESH_TOKEN` | Integration tests | Google OAuth refresh token used by automated tests when needed. |

The environment-file secrets should contain backend runtime values such as database settings, storage settings, OAuth settings, and collaborator configuration. The GKE workflow overrides:
...
Keep `COLLABORATOR` in the environment secret if the backend environment uses collaborator-specific processors or configuration.
...
| Name | Purpose |
| --- | --- |
| `ENABLE_GKE_DEPLOYMENTS` | Enables automatic GKE deploys for all environment workflows that check this variable. |
...
When adding a new environment, add the matching secret and variable only if the workflow is configured to read it.
...
| `<STATE>_BACKEND_URL` | State frontend deployments | Backend URL for a collaborator frontend, such as `CA_BACKEND_URL`, `ISGS_BACKEND_URL`, `NEWTS_BACKEND_URL`, or `OSAGE_BACKEND_URL`. |
