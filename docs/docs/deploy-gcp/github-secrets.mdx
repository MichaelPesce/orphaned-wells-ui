...
| `<COLLABORATOR>_ENV` | GKE deployment | Runtime `.env` content for each dispatch-supported backend collaborator. The current dispatch workflow reads `STAGING_ENV`, `CA_ENV`, `ISGS_ENV`, `NEWTS_ENV`, and `OSAGE_ENV`. |
