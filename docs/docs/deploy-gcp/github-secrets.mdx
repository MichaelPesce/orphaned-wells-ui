...
| `<COLLABORATOR>_ENV` | GKE deployment | Runtime `.env` content for each backend collaborator, such as `STAGING_ENV`, `CA_ENV`, `ISGS_ENV`, `NEWTS_ENV`, and `OSAGE_ENV`. |
...
When adding a new collaborator, add the matching secret and variable only if the workflow is configured to read it.
