...
description: Legacy Compute Engine VM deployment path for OGRRE backend environments.
sidebar_position: 7
---
...
:::warning Legacy path
The current backend deployment path is GKE. Use this page only for existing Compute Engine VM operations, rollback context, or environments that have not been migrated.
:::
...
On the VM, add or copy the Docker Compose and nginx configuration files for the environment.
...
sudo docker compose run --rm certbot certonly --webroot -w /var/www/certbot \
  ...
  -d <env>-server.uow-carbon.org --force-renewal
```
