...
Run the focused project, record, and schema loading checks for the schema mode
configured in `deployment/.env.e2e`:

```console
npm run e2e:schema-modes
```
...
The local `e2e:open`, `e2e:run`, `e2e:smoke`, `e2e:schema-modes`, `e2e:full`, and `e2e:ci` scripts derive Cypress defaults from the E2E Docker environment:
...
| `CYPRESS_USE_DB_PROCESSORS` | `USE_DB_PROCESSORS` from `deployment/.env.e2e`, default `false` |
...
The full suite includes database schema creation and editing tests. Set
`USE_DB_PROCESSORS=true` in `deployment/.env.e2e`, recreate the backend, and then
run it:

```console
npm run docker:e2e:recreate-backend
npm run e2e:full
```
