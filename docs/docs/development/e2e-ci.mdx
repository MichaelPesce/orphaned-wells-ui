...
The frontend repository includes `.github/workflows/frontend-tests-dispatch.yml` for reusable frontend validation. The workflow runs unit tests, starts the isolated Docker E2E stack, verifies service health, tests loading in both schema modes, and runs Cypress with CI-oriented settings.

CI uses the same E2E harness as local development. It runs
`npm run e2e:schema-modes` in package mode, recreates the backend, and then runs
`npm run e2e:ci` in database mode. Both Cypress commands use Chrome in CI.
...
5. Start the isolated E2E Docker stack with `USE_DB_PROCESSORS=false`.
6. Wait for `http://localhost:8002/docs` and `http://localhost:3001`.
7. Run the focused schema-mode loading contract.
8. Recreate the backend with `USE_DB_PROCESSORS=true`.
9. Verify the backend reports database mode and run the full E2E suite.
10. Upload Cypress screenshots, videos, and backend logs.
11. Stop and remove the E2E stack with `npm run docker:e2e:clean`.
...
CYPRESS_USE_DB_PROCESSORS=<USE_DB_PROCESSORS from deployment/.env.e2e>
...
The package-mode pass verifies loading only. The workflow switches explicitly to
`USE_DB_PROCESSORS=true` before the full suite so database schema mutation tests
still run once.
...
CI builds the backend from source. Push/PR runs use backend `main`; manually
dispatched App Tests runs accept `backend_repository` and `backend_ref` to test
coordinated frontend/backend changes. Local runs can leave `BACKEND_MODE=auto`
to use a sibling backend checkout or set `BACKEND_MODE=source` explicitly.
Use an updated checkout when testing lightweight loading and missing-schema
fallbacks. Image-mode runs require a newly published backend image with those
changes; restarting an old image does not update its code.
